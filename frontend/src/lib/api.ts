...
export const posts = {
  getAll: (page = 0, limit = 10) =>
    api.get<PaginatedResponse<Post>>("/api/posts", { params: { page, limit } }),
  ...
  getUserPosts: (username: string, page = 1, limit = 10) =>
    api.get<PaginatedResponse<Post>>(`/api/posts/user/${username}`, {
      params: { page: page - 1, limit },
    }),
  getHashtagPosts: (tag: string, page = 0, limit = 10) =>
    api.get<PaginatedResponse<Post>>(`/api/posts/hashtag/${tag}`, {
      params: { page, limit },
    }),
